        ...
        if (upgrade_flag) {
            ...
            if (upg->func->upgrade) {
                ...
                if (ret < 0) {
                    ...
                    #ifdef SUPPORT_READ_TP_VERSION
                        fts_data->fw_ver[0] = ver;
                        fts_param_get_ver_in_tp(client, &ver_id);
                        fts_data->para_ver_id = ver_id;
                        memset(fw_version, 0, sizeof(fw_version));
                        sprintf(fw_version, "[FW]0x%02x.0x%02x,[IC]FT%02x%02x",ver,ver_id,fts_data->ic_info.ids.chip_idh, fts_data->ic_info.ids.chip_idl);
            ...
            if (upg->func->param_upgrade) {
                if (fts_param_need_upgrade(client)) {
                    ...
                    if (ret < 0) {
                        ...
                        #ifdef SUPPORT_READ_TP_VERSION
                          fts_data->para_ver_id = ver;
                          fts_fwupg_get_ver_in_tp(client, &ver_id);
                          fts_data->fw_ver[0] = ver_id;
                          memset(fw_version, 0, sizeof(fw_version));
                          sprintf(fw_version, "[FW]0x%02x.0x%02x,[IC]FT%02x%02x",ver_id,ver,fts_data->ic_info.ids.chip_idh, fts_data->ic_info.ids.chip_idl);
                          update_tp_fm_info(fw_version);
                        #endif
                        break;
                    }
...
static int fts_fwupg_get_fw_file(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("success to read vendor id:0x%x", vendor_id);
