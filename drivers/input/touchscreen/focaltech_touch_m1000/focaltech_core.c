...
static int fts_ts_resume(struct device *dev);
 ...
 static void fts_update_fw_ver(struct fts_ts_data *data)
{
	...
	reg_addr = FTS_REG_IDE_PARA_VER_ID;
	err = fts_i2c_read(client, &reg_addr, 1, &data->para_ver_id, 1);
	if (err < 0)
		dev_err(&client->dev, "fw paramcfg version read failed");


	dev_info(&client->dev, "Firmware version = %d.%d.%d,paramcfg = 0x%02x\n",
		data->fw_ver[0], data->fw_ver[1], data->fw_ver[2],data->para_ver_id);
}
...
//add for tp info start
      fts_update_fw_ver(ts_data);
#ifdef SUPPORT_READ_TP_VERSION	
      memset(fw_version, 0, sizeof(fw_version));
      sprintf(fw_version, "[FW]0x%02x.0x%02x,[IC]FT%02x%02x", ts_data->fw_ver[0],ts_data->para_ver_id,ts_data->ic_info.ids.chip_idh, ts_data->ic_info.ids.chip_idl);
